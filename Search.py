...
#     server.quit()
#     tkinter.messagebox.showerror('', '验证码已发送！')
#
#
# def generate():
#     email_test = ''
...
#             server.sendmail(from_addr, to_addr, msg.as_string())
#             server.quit()
#             tkinter.messagebox.showerror('', '您的密码已发送到您的邮箱中！')
...
def forgot(email, email_test):
    ...
    server.sendmail(from_addr, to_addr, msg.as_string())
    server.quit()
    tk.messagebox.showerror('', '验证码已发送！')
...
def send():
    ...
    def confirm():
        ...
        if em1.get() not in emails:
            ...
            server.sendmail(from_addr, to_addr, msg.as_string())
            server.quit()
            tk.messagebox.showerror('', '您的密码已发送到您的邮箱中！')
            window_forgot.destroy()

    em = tk.Label(window_forgot, text='邮箱：', )
    em1 = tk.Entry(window_forgot, font=('Arial', 14))
    emts = tk.Button(window_forgot, text='获取', command=confirm, font=('Arial', 14))
    em.place(x=33, y=15)
    em1.place(x=75, y=15)
...
def sign_up():
    ...
    u1 = tk.Entry(window_sign_up, font=('Arial', 14))
    p1 = tk.Entry(window_sign_up, show='*', font=('Arial', 14))
    cp1 = tk.Entry(window_sign_up, show='*', font=('Arial', 14))
    button = tk.Button(window_sign_up, text='注册', command=confirm, font=('Arial', 14))
    em1 = tk.Entry(window_sign_up, font=('Arial', 14))
    emts = tk.Button(window_sign_up, text='获取', command=get_emts, font=('Arial', 14))
    et1 = tk.Entry(window_sign_up, font=('Arial', 14))
    u.place(x=20, y=15)
    et.place(x=20, y=155)
    p.place(x=33, y=50)
    cp.place(x=7, y=85)
...
def search():
    ...
    label.pack()
    i = tk.Entry(window, width=50, font=('Arial', 14))
    b1 = tk.Button(window, text='搜索', command=search_res, font=('Arial', 14))
...
b = tk.Button(window, text='登录', command=log, font=('Arial', 14))
forget = tk.Button(window, text='忘记密码？', command=send, font=('Arial', 14))
signup = tk.Button(window, text='注册', command=sign_up, font=('Arial', 14))
b.place(x=210, y=85)
